...
namespace LinPro.Datos
{
    public class DatosPersonalUsuario
    {
        ...
        public ComboPuestos LlenaComboPuesto()
        {
            ...
            DataTable dt = new DataTable();
            SqlConnection connection = null;
            List<CamposPuesto> composList = new List<CamposPuesto>();
            ...
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    CamposPuesto reg = new CamposPuesto();
                    reg.idPuesto = Convert.ToInt32(row["idPuesto"].ToString());
        ...
        {
            ComboRoles listaRoles = new ComboRoles();
            ...
            try
            {
                ...
                listaRoles.listaRoles = composList.ToArray();
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public ComboEstados LlenaComboEstados()
        {
            ...
            try
            {
                using (connection = Conexion.ObtieneConexion("ConexionBD"))
                {
                    SqlDataReader consulta;
                    connection.Open();
                    consulta = Ejecuta.ConsultaConRetorno(connection, "[dbo].[Usp_CombEstados]");
                    dt.Load(consulta);
                    connection.Close();
                }
                foreach (DataRow row in dt.Rows)
