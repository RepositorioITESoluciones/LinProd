    ...
    $('#btnbuscar').click(function () {
        ...
        if (valido) {
            $.ajax({
                async: false,
                type: 'POST',
                url: 'WSLinPro.asmx/LlenaTablaEnergiaXEquipos',
                ...
                success: function (response) {
                    ...
                    $.each(response, function (row, index) {
                        $.each(index.ListaRegistros, function (r, arr) {
                            datos.push([
                                arr.departamento.nombreDepartamento, arr.maquina.Descripcion,
                                arr.KWH_pza, arr.KWH_turno, arr.fechas
                                       ]);
                        });
                    });
                }
            ...
            //$('#tablasReportes').show();
            var otable = $('#detalleEnergia').DataTable({
                "aLengthMenu": [
                    [5, 10, 25, 50],
                    [5, 10, 25, 50]
                ],
                ...
                "preDrawCallback": function () {
                    if (!responsiveHelper_datatable_fixed_column) {
                        responsiveHelper_datatable_fixed_column = new ResponsiveDatatablesHelper(
                            $('#detalleEnergia'), breakpointDefinition);
                    }
                },
            ...
            $('#detalleEnergia tbody').on('click', 'tr', function () {
                if ($(this).hasClass('selected')) {
                    $(this).removeClass('selected');
                } else {
                    $('#detalleEnergia').DataTable().$('tr.selected').removeClass(
                        'selected');
