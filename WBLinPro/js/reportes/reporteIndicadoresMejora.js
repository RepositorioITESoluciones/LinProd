...
function initDataTableEficiencia() {
    $('#FormReporte').show();
    $('#tablasReporte').hide();
    ...
    $('#btnbuscar').click(function () {
        ...
        if (valido) {
            $.ajax({
                async: false,
                type: 'POST',
                url: 'WSLinPro.asmx/IndicadoresEficiencia',
                ...
                success: function (response) {
                    ...
                    $.each(response, function (row, index) {
                        $.each(index.ListaRegistros, function (r, arr) {
                            datos.push([
                                arr.maquina.Descripcion, arr.fechas, arr.piezasProducidas,
                                arr.piezasDefectuosas, arr.PiezasBuenas, arr.eficiencia
                                       ]);
                        });
                    });
                }
            ...
            //$('#tablasReportes').show();
            var otable = $('#detalleIndicadores').DataTable({
                "aLengthMenu": [
                    [5, 10, 25, 50],
                    [5, 10, 25, 50]
                ],
                ...
                "preDrawCallback": function () {
                    if (!responsiveHelper_datatable_fixed_column) {
                        responsiveHelper_datatable_fixed_column = new ResponsiveDatatablesHelper(
                            $('#detalleIndicadores'), breakpointDefinition);
                    }
                },
                ...
                columns: [
                    ...
                    {
                        title: "Piezas con Error"
                    },
                    {
                        title: "Piezas Correctas"
                    },
                    {
                        title: "Eficiencia"
                    }
            ...
            $("#detalleIndicadores thead th input[type=text]").on(
                'keyup',
                function (e) {
            ...
            $('#detalleIndicadores tbody').on('click', 'tr', function () {
                if ($(this).hasClass('selected')) {
                    $(this).removeClass('selected');
                } else {
                    $('#detalleIndicadores').DataTable().$('tr.selected').removeClass(
                        'selected');
